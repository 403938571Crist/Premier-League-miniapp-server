...
| `REDIS_HOST` | Redis 地址（如有） | `10.x.x.x` |
| `REDIS_PORT` | Redis 端口 | `6379` |
| `REDIS_PASSWORD` | Redis 密码（如有） | `你的密码` |
...
> 💡 如果没有 Redis，可以把 `REDIS_HOST` 留空，并修改 `application.yml` 关闭 Redis 相关配置，或改用本地缓存（见下方【去掉 Redis】）。
...
### Q3：没有 Redis 怎么办？
A：可以购买腾讯云 Redis，也可以把缓存改成 Spring 本地缓存。本地缓存改造方案：
1. 在 `pom.xml` 中添加 `caffeine` 依赖；
2. 修改 `RedisConfig.java` 为 `CacheConfig.java`；
3. 将 `@Cacheable` 的 `cacheManager` 改为 Caffeine。
